...
export default class YcDialog extends LitElement {
  ...
  @property({type: String, reflect: true, attribute: 'yc-title'})
  override title = '提示';
      ...
      <div class="dialog">
        ${dialogTypeData.name
          ? html`<yc-icon
              ...
              size="${30}"
              class="dialog-type"
              name=${dialogTypeData.name}
              color=${dialogTypeData.color}
            ></yc-icon>`
          : null}
        <div class="dialog-content">
          <div class="dialog-title" id="title">
          ...
          <div class="dialog-body">
            <slot></slot>
            ${this.type === 'prompt'
              ? html`<yc-input class="prompt" id="promptInput"></yc-input>`
              : ''}
          </div>
          <div class="dialog-footer">
            <slot name="footer">
              <yc-button
                id="btn-cancel"
                type="flat"
                @click=${this.cancelBtnHandler}
                >${this.cancelText}</yc-button
              >
              <yc-button
                id="btn-submit"
                .loading=${this.loading}
                type="primary"
                @click=${this.submitBtnHandler}
                >${this.okText}</yc-button
              >
            </slot>
          </div>
        </div>
  ...
  override firstUpdated(
    ...
    const _btnSubmit: YcButton | null =
      this.renderRoot.querySelector('#btn-submit');
  ...
  static prompt = function () {
    ...
    if (typeof arguments[0] === 'object') {
      ...
      dialog.innerHTML = content || '';
      const input: YcInput = dialog.renderRoot.querySelector(
        'yc-input'
      ) as YcInput;
