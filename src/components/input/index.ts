...
class MinInputClass extends LitElement {
  ...
  customValidateMethod?: CustomValidateMethode;
...
class YcInput extends MinInputClass {
  ...
  private typePassword() {
    const btn: YcButton = this.renderRoot.querySelector(
      '#eye-icon'
    ) as YcButton;
  ...
  private static NUMBERINPUTARRAY: string[] = [
    '0',
    '1',
    '2',
    '3',
  ...
  override update(changedProperties: Map<string | number | symbol, unknown>) {
    ...
    if (changedProperties.has('throttle') && this.throttle !== undefined) {
      ...
      changedProperties.has('debounce') &&
  ...
  override render() {
    return html`<yc-tips .tips=${this.tips} id="tips">
      ${this.leftIcon
        ? html`<yc-icon name="${this.leftIcon}" class="leftIcon"></yc-icon>`
        : ''}
      <input
        id="input"
        part="input"
        name="${ifDefined(this.name)}"
        placeholder="${ifDefined(this.label ? this.label : this.placeholder)}"
        .value="${this.value}"
        @input="${this._processInput}"
      ...
      ${this.label
        ...
        : ''}
      ${this.firstTypePassword
        ? html`<p-button
            class="eye-icon"
            id="eye-icon"
            part="show-password-icon"
            @click="${this.typePassword}"
            icon="eye-close"
            type="flat"
            shape="circle"
          ></p-button>`
      ...
      ${this.type === 'search'
        ? html`<p-button
          ...
          ></p-button>`
        : ''}
      ${this.type === 'number' && this.showStep
        ? html`<div class="btn-right btn-number">
            <p-button
              id="btn-add"
              icon="up"
              part="number-up"
        ...
        : ''}
      ${this.rightIcon
        ? html`<yc-icon name="${this.rightIcon}" class="rightIcon"></yc-icon>`
        : ''}
      <slot></slot>
    </yc-tips>`;
  }
