...
export function HSVaColor(h = 0, s = 0, v = 0, a = 1) {
  const mapper =
    (original: Number[], next: NextCovertor) =>
    (precision = 0) => {
      return next(
        ~precision
  ...
  const that = {
    ...
    toHSVA() {
      ...
      hsva.toString = mapper(
        hsva,
        // @ts-ignore
        (arr: number[]) => `hsva(${arr[0]}, ${arr[1]}%, ${arr[2]}%, ${that.a})`
      );
      return hsva;
    },
    ...
    toHSLA() {
      ...
      hsla.toString = mapper(
        hsla,
        // @ts-ignore
        (arr: number[]) => `hsla(${arr[0]}, ${arr[1]}%, ${arr[2]}%, ${that.a})`
      );
      return hsla;
    },
    ...
    toRGBA() {
      ...
      rgba.toString = mapper(
        rgba,
        // @ts-ignore
        (arr: number[]) => `rgba(${arr[0]}, ${arr[1]}, ${arr[2]}, ${that.a})`
      );
      return rgba;
    },
